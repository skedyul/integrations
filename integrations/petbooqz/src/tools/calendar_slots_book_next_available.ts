...
import { petbooqzBookingTouchPoints } from '../lib/touch_points'
...
> = {
  ...
  handler: async (input, context) => {
    ...
    const availabilityClient = createClientFromEnv(context.env)
    const rawDatetimes = await fetchAvailableDatetimes(
      availabilityClient,
      input.calendar_id,
      input.dates,
    )
